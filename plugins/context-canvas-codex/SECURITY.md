...
- Plugin discovery, installation, and a trusted hash do not prove hooks executed in the current task. A later prompt may demonstrate turn-hook pickup on builds that reload trusted hook configuration; verify a new `SessionStart` opaque ID and a harmless-call snapshot in a fresh task for the stronger end-to-end check.

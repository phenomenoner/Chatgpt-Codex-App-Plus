...
`SessionStart` and `UserPromptSubmit` are read-only. Both derive the current opaque ID from the exact hook-supplied session ID. The turn hook validates that a prompt field exists but never stores, echoes, searches, or interprets its contents; an internal failure emits no context and cannot block the prompt. `PostToolUse` persistence is off by default. `snapshot_capture_next` requires a nonempty exact tool name; omission, empty input, and wildcard scope fail before creating or replacing a request. An otherwise canonical wildcard request persisted by the predecessor release never matches a callback; it is retained until explicit cancellation, exact replacement, or expiry. Other malformed wildcard-like state fails closed without deletion. The next exactly matching non-Canvas callback may archive a policy-sanitized historical copy of the model-facing tool input and response supplied by Codex. The request is consumed once. A Bash non-zero command exit can still be observed through this callback. Dispatch or handler failures that produce no callback payload are absent. The hook emits no model-facing output and fails open, so capture failure cannot replace the original tool result.

The compatibility installer is an explicit configuration write. It merges exact `SessionStart`, `UserPromptSubmit`, and `PostToolUse` groups into the user's Codex `hooks.json`, copies the audited script to a stable Codex-home path, preserves peer handlers, and keeps a hash-addressed backup. Only exact manifest-bound v1 one-hook, v2 two-hook, and v3 three-hook definitions are recognized. Their canonical targets, exact fields and event sets, coherent lowercase digests, and live adapter bytes are checked before replacement; a third adapter digest, duplicate, ambiguity, foreign drift, alias, or non-regular file is rejected without changing the pre-existing owned bytes. Explicit install may adopt already-exact current groups because it first preserves the hooks document and then establishes the current adapter and manifest. Uninstall requires a canonical supported manifest or the exact current adapter before deleting any existing managed group; structural equality or possession of the host lock alone is not deletion authority.

`install`, `check`, and `uninstall` hold one Codex-home-scoped exclusive OS file lock across their complete ownership read and multi-file transition. The stable empty lock file may remain after retirement to preserve one lock identity. It is host coordination metadata for cooperating installer processes only: it is not Canvas product state, task authority, an authorization boundary, or protection from same-user software that ignores the lock.
...
- V2 objects bind each generated data-URL slot to an exact root-relative path, digest, byte length, media type, and policy outside caller JSON; literal marker-shaped dictionaries therefore remain literal through read, export, pin, and GC. Markerless V1 objects remain readable, while structurally ambiguous V1 blob markers fail closed;
...
Canonical JSON objects and extracted blobs are content-addressed with lowercase SHA-256 identities and deterministically gzip-compressed. Full read, export, promotion, and GC preflight verify compression, canonical digest spelling, canonical object bytes, manifest-declared object and blob lengths, schema, manifest path/session identity, path-bound object relationships, and declared blob policy. Bounded list operations validate bound manifests without loading bodies. Repeated identical payloads deduplicate without losing per-call event manifests. Reference manifest/body creation uses the existing cross-process lock; a one-sided interrupted pair is completed only by a deterministic retry that verifies the surviving owned bytes, while unverified one-sided state remains intact and visible as bounded corruption.

...
| Explicit references | `reference_put` stores bounded policy-redacted text; content hits from `reference_search` include digest-bound UTF-8 byte ranges and a directly usable bounded `reference_read` hint; explicit `reference_preview` derives ephemeral byte-exact `log-v1` or `search-results-v1` slices; `reference_delete` removes one reference idempotently. An interrupted deterministic pair write is either verified and completed on retry or reported as bounded `incomplete_pair` corruption without deleting unproven bytes. Summary-only hits do not read the body. |
| One-shot historical tool snapshots | Capture is off by default. `snapshot_capture_next` requires one nonempty exact tool name and arms one expiring request for that matching non-Canvas callback; omission, empty input, and the former implicit wildcard are rejected before persistence. An otherwise canonical wildcard request left by the predecessor release is non-authorizing compatibility state: it can only be cancelled, replaced by an exact request, or removed on expiry. Other malformed requests remain intact and fail closed. |
| Content-addressed storage | Canonical JSON and extracted data-URL blobs are SHA-256 addressed, deterministically gzip-compressed, integrity-checked, and deduplicated. V2 snapshot objects keep generated blob provenance in path-bound root metadata, so an ordinary literal `$snapshot_blob`-shaped dictionary remains ordinary JSON. Supported textual base64 and percent-encoded data URLs, including unquoted MIME parameters, are decoded and redacted before persistence; opaque binary media is stored unchanged and explicitly labelled uninspected. |
...
The installer migrates only exact manifest-bound v1 SessionStart-only, v2 SessionStart-plus-PostToolUse, and v3 three-hook generations, including the predecessor 0.4 definitions. Each manifest must use its exact field and event set, canonical targets, coherent lowercase SHA-256 digests, and a live adapter matching either the manifest-bound bytes or the current candidate. A third digest, duplicate or marker-only handler, ambiguous ownership, or foreign path is refused before any owned configuration bytes are changed. Peer handlers are preserved, and interrupted script-first recovery is allowed only when the unmanifested bytes exactly match the current source. Explicit install may adopt already-exact current groups by first backing up the hooks document and then establishing the current adapter and manifest. Uninstall requires either a canonical supported manifest or the exact current adapter before deleting any existing managed group; structural equality alone is not deletion authority.

`install`, `check`, and `uninstall` share one Codex-home-scoped exclusive OS lock across the complete adapter, manifest, and hook transition. The stable empty lock file may remain after uninstall so cooperating processes cannot split lock identities; it is host coordination metadata only, not Canvas state, authorization, or a workflow gate. Re-run `install` after each plugin upgrade.
...
For retirement, run `python -I scripts/install_context_canvas_hook.py uninstall` to remove only managed user-hook groups whose ownership is witnessed by the canonical manifest or exact current adapter, and to recoverably retire owned files. Disable the MCP server and skill before archiving the plugin and its data root. External evidence targets remain outside Canvas ownership and are never deleted by these commands.

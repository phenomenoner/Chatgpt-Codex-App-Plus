...
The bundle includes the `context-canvas-codex` plugin and the optional `operate-a2a-superhub` skill. Context Canvas provides optional session navigation for goals, decisions, progress, dependencies, blockers, and next steps; explicit text references with digest-bound search ranges, chunk reads, and deterministic previews; default-off, one-shot historical tool snapshots; and an evidence-first reflection skill for meaningful trajectory checkpoints. Reflection is advisory and returns one bounded disposition; it is not a controller or authority. Hook-derived IDs are transport provenance for Canvas actions, not task authority. Missing Canvas state never blocks the underlying work.
...
Context Canvas 0.7 keeps the 0.6 Canvas core and adds the separate `context-canvas-reflection` v0 preview skill. It runs one evidence-first pass only at a meaningful checkpoint such as a repeated same-cause failure, a contradicted critical assumption, local green with real-use failure, material scope drift or phase uncertainty, an authority-sensitive next effect that lacks current owner approval, or explicit user doubt. First ordinary failures, healthy progress, fixed cadence, task size, and Canvas availability are non-triggers. The result is one bounded `CONTINUE`, `INVESTIGATE`, or `ESCALATE` disposition; the main agent, harness, or user keeps every planning and effect decision. Implicit invocation is disabled in the public plugin by default; users can call `$context-canvas-reflection` explicitly or opt in through a bounded policy in their own `AGENTS.md`.

The Canvas core does not equate “preserve useful context” with “create a semantic node.” `reference_put`, `reference_search`, `reference_preview`, `reference_read`, and `reference_delete` provide explicit redacted text offload with bounded native retrieval. Content-search hits carry the source digest, exact UTF-8 byte range, and a directly usable read hint. `reference_preview` explicitly applies either `log-v1` or strict line-oriented `search-results-v1` and returns ephemeral exact source slices under a caller-supplied budget. It never runs automatically, rewrites the stored body, creates a derived cache, or substitutes a lossy summary for the original. `PostToolUse` persistence is off by default: `snapshot_capture_next` arms one expiring request, exact tool mismatches do not consume it, Canvas self-tools are ignored, and a match consumes it once. The captured model-facing payload still uses the declared sanitization, content-addressing, dedupe, TTL, pin, and integrity-checked GC policies. Snapshot bodies stay outside task-map search and lifecycle injection; an explicit MCP/CLI read returns bounded chunks, while complete file export remains CLI-only.
